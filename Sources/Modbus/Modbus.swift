...
let EMBXILFUN  = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_ILLEGAL_FUNCTION)
let EMBXILADD  = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS)
let EMBXILVAL  = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE)
let EMBXSFAIL  = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_SLAVE_OR_SERVER_FAILURE)
let EMBXACK    = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_ACKNOWLEDGE)
let EMBXSBUSY  = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_SLAVE_OR_SERVER_BUSY)
let EMBXNACK   = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_NEGATIVE_ACKNOWLEDGE)
let EMBXMEMPAR = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_MEMORY_PARITY)
let EMBXGPATH  = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_GATEWAY_PATH)
let EMBXGTAR   = MODBUS_ENOBASE + Int32(MODBUS_EXCEPTION_GATEWAY_TARGET)
...
public final class Modbus {
    ...
    func read<Pointee: UnsignedInteger, ReturnType>(from: Int, count: Int, function: ReadFunction<Pointee>, mapper: (Pointee) -> ReturnType) throws -> [ReturnType] {
        ...
        if function(mb, Int32(from), Int32(count), dest) == -1 {
            throw ModbusError.unknown(999) // TODO: Find the new way to use errno
        }
